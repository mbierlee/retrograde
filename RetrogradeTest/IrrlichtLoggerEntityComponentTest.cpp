...
TEST(IrrlichtLoggerEntityComponentTest, testLogging) {
	...
	EXPECT_CALL(logger, log("Test Info", irr::ELL_INFORMATION))
		.WillOnce(Return());
	EXPECT_CALL(logger, log(L"Test Error", irr::ELL_ERROR))
		.WillOnce(Return());
	EXPECT_CALL(logger, log(L"Test Warning", irr::ELL_WARNING))
		.WillOnce(Return());

	Engine::EntityComponents::IrrlichtLoggerEntityComponent irrlichtLoggerEntityComponent(&logger);
	irrlichtLoggerEntityComponent.log("Test Info");
	irrlichtLoggerEntityComponent.log(L"Test Error", irr::ELL_ERROR);
	irrlichtLoggerEntityComponent.log(L"Test Warning", irr::ELL_WARNING);
}