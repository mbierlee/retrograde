...
Engine::EntityFactories::DebugEntityFactory::DebugEntityFactory(std::shared_ptr<irr::IrrlichtDevice> device,
		...
		device(device), physicsService(physicsService) {
}
...
std::shared_ptr<Engine::Framework::IEntity> Engine::EntityFactories::DebugEntityFactory::makeDebugPhysCube() {
	...
	irr::core::vector3df meshSize = Engine::makePhysicsToWorldsizeVector(1.f, 1.f, 1.f);
	irr::scene::IMesh* visualMesh = device->getSceneManager()->getGeometryCreator()->createCubeMesh(meshSize);
