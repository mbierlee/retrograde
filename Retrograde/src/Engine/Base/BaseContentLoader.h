...
template<class T>
class BaseContentLoader: public Engine::Framework::IContentLoader<T> {
private:
...
public:
	...
	virtual std::shared_ptr<T> requestContent(const irr::core::stringw& fileName) override {
	...
	virtual void releaseContent(const irr::core::stringw& fileName) override {
		LoadedContentIterator it;
		it = loadedContent.find(fileName);
		if (it != loadedContent.end()) {
	...
	virtual irr::u32 cacheSize() override {
