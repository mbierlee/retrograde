...
Engine::Base::BaseGame::BaseGame(std::shared_ptr<irr::IrrlichtDevice> device, std::shared_ptr<Engine::Framework::IEntityService> entityService,
		...
		isExitRequested(false), device(device), entityService(entityService), eventService(eventService), entityFactoryService(entityFactoryService), inputService(
				inputService), lastFrameTime(0), frameTime(0), deltaTime(0) {
	if (device) {
