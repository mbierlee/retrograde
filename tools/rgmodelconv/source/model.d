 ...
 * base color (albedo), normal, metallic-roughness, occlusion and emissive textures plus the
 * base color, metallic, roughness and occlusion strength factors and the emissive factor with
 * its strength.
 */
struct MaterialInfo {
    ...
    TextureRef emissiveTexture; /// The externally referenced emissive map (path + filters), whose RGB says where the surface glows; `path` is "" when the material has none. Only written for material types that carry an emissive payload.
    float[3] emissiveFactor = [0.0f, 0.0f, 0.0f]; /// glTF `material.emissiveFactor`: the RGB light the material gives off by itself, multiplied over the map above where there is one. Defaults to black, which is also what glTF means by an absent factor - a material that emits nothing. Only written for material types that carry an emissive payload.
