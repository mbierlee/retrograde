...
Total file size = 18 + sum_meshes(14 + hasBounds × 24 + vertexCount × 24 + faceCount × 12 + uvChannelCount × vertexCount × 8 + hasNormals × vertexCount × 12 + hasTangents × vertexCount × 16) + sum_materials(materialEntrySize) + sum_textures(textureEntrySize), where `hasBounds`, `hasNormals` and `hasTangents` are 1 when the corresponding attribute flag is set and 0 otherwise.
...
| 0x0E   | 24                               | Bounds  | Bounds (optional)                |
| ...    | vertexCount × 24                 | Vertex  | Vertex data                      |
...
A bitfield marking which optional blocks the mesh carries: the bounds block in its header, and the per-vertex attribute blocks that follow the UV channel data. Bits not listed here are reserved; they must be written as 0 and a file that sets any of them is rejected.

| Bit | Mask | Description                                                    |
| --- | ---- | -------------------------------------------------------------- |
| 0   | 0x01 | Normal data is present.                                        |
| 1   | 0x02 | Tangent data is present.                                       |
| 2   | 0x04 | Bounds are present.                                            |
...
Neither per-vertex attribute is generated by the toolchain — both are carried over from the source model only when it supplies them. Bounds are the opposite: `rgmodelconv` computes and writes them for every mesh unless run with `--no-bounds`, and the engine computes them itself for a mesh whose file leaves them out.

### Bounds (24 bytes)

Present only when bit 2 of the attribute flags is set, immediately after the material index and before the vertex data. It sits in the header rather than after the vertex blocks so that a reader can pick up a mesh's extent without walking its vertices, which is what the block is for.

The axis-aligned box enclosing every vertex of the mesh, in model space, as its two extreme corners: the corner with the smallest coordinate on every axis, then the corner with the largest. The engine uses it for broad checks such as whether an entity may be in view, transforming it along with the entity. A mesh without vertices stores all zeros, which is also what a reader computes for one.

The block is optional so that a file can leave the work to the loader: a reader that finds no bounds computes the tightest box around the vertices it reads, so a mesh always ends up with bounds either way. When present the values are used as stored, so a writer may pad them out, but must not write a box smaller than the vertices it encloses.

| Offset | Size | Type  | Description        |
| ------ | ---- | ----- | ------------------ |
| 0x00   | 4    | float | Min X coordinate   |
| 0x04   | 4    | float | Min Y coordinate   |
| 0x08   | 4    | float | Min Z coordinate   |
| 0x0C   | 4    | float | Max X coordinate   |
| 0x10   | 4    | float | Max Y coordinate   |
| 0x14   | 4    | float | Max Z coordinate   |
