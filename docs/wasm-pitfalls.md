...
## Only the allocator suite may wipe the heap between tests
...
The messages appear *after* `test()` has printed the name but before the body
can have allocated anything, and the **first** test of a section is clean while
every later one warns — what is being freed is whatever the previous test left
in the globals. With a `LinkedList` global it is not benign: `clear()` walks
`node = node.next` through wiped memory and the suite dies with a fatal
...
A heap wipe running between tests while module globals live on. Module-level
globals are not in the heap — they live in the data segment, which
`wipeHeap()` does not touch. So a global `HashMap`/`Array`/`Queue`/`LinkedList`
keeps its `buckets`/`items`/`head` pointers into heap memory that has just been
zeroed and handed back to the allocator. The next test's `resetX()` then calls
`clear()` through those dangling pointers, and the allocator correctly rejects
freeing memory it considers free.

`retrograde.std.test.test()` used to do exactly this for every test under
`WasmMemTest`, which made every reset helper in the engine wrong on WASM and
right on native. It no longer does.

### Fix / rule

**The heap wipe belongs to the allocator suite alone.** `runWasmMemTests()` in
`source/retrograde/wasm/memory.d` runs its tests through a private `memTest`
wrapper that does `wipeHeap(); initializeHeapMemory();` first, because those
tests leak whole free runs on purpose, clobber the page map and shift
`heapOffset`. The suite ends with one more `resetHeap()` so the suites after it
inherit a sane allocator.

Nothing else wipes. Every other suite runs on a live heap, exactly like native,
so a test-only reset is just what it is on native:
...
Do not reintroduce a wipe in `test()` or anywhere else that runs across suites:
it would silently invalidate every module global in the engine and bring the
symptom above back. If a suite genuinely needs a pristine allocator, wrap its
own tests the way `memTest` does and restore the heap when it is done.

### How to diagnose

Run `cd wasmtest && make run-tests-headless` and check which section any
allocator complaint falls in — a fair number are legitimate, emitted by the
negative tests in `source/retrograde/wasm/memory.d` that deliberately double-free,
and those all sit under `-- Low-level WASM memory tests --`:

```
awk '/^-- .* --$/{s=$0} /^free:|^realloc:|^memmove:/{print s" | "$0}' out.txt | sort | uniq -c
```

Anything attributed to another section is a real problem: either this pitfall
has come back, or the code under test has a genuine double free.
